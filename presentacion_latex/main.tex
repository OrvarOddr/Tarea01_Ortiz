...
\definecolor{codigofondo}{HTML}{F7F1E8}
\definecolor{codigoborde}{HTML}{C97A3A}

\lstdefinestyle{codexpython}{
  language=Python,
  basicstyle=\ttfamily\tiny,
  keywordstyle=\bfseries\color{cobreoscuro},
  stringstyle=\color{verdeoxido},
  commentstyle=\color{cobreoscuro},
...
\setbeamercolor{headlinebar}{fg=white,bg=carbon}
\setbeamercolor{footlinebar}{fg=arena,bg=carbon}
  ...
  \begin{tikzpicture}[baseline=-0.45ex]
    \foreach \i in {1,...,\inserttotalframenumber} {
      \ifnum\i=\insertframenumber
        \fill[cobrebrillo] (\i*0.145,0) circle (1.55pt);
      \else
        \fill[white,opacity=0.34] (\i*0.145,0) circle (1.18pt);
      \fi
    }
  \end{tikzpicture}%
}
...
\setbeamertemplate{headline}{%
  \begin{beamercolorbox}[wd=\paperwidth,ht=4.4ex,dp=1.2ex,leftskip=0.34cm,rightskip=0.34cm]{headlinebar}
    \raisebox{-0.58ex}{\includegraphics[height=3.6ex]{imagenes/logo_universidad_biobio.png}}%
    \hspace{0.22cm}{\color{white}\scriptsize\bfseries Ciencia de Datos para Economía y Gestión}%
    \hfill{\color{arena}\scriptsize Tarea 1 | Limpieza de datos mineros}%
    \hspace{0.24cm}\progressdots
  \end{beamercolorbox}
}

\setbeamertemplate{footline}{%
  \begin{beamercolorbox}[wd=\paperwidth,ht=2.9ex,dp=1.15ex,leftskip=0.45cm,rightskip=0.45cm]{footlinebar}
    \color{arena}Producción de cobre en Chile \hfill Ciencia de Datos para Economía y Gestión \hfill \insertframenumber/\inserttotalframenumber
  \end{beamercolorbox}
}
...
\newcommand{\copperband}{%
  \begin{tikzpicture}[remember picture,overlay]
    \fill[carbon,opacity=0.94] (current page.north west) rectangle ([yshift=-0.48cm]current page.north east);
    \node[anchor=west] at ([xshift=0.34cm,yshift=-0.24cm]current page.north west) {
      \includegraphics[height=0.34cm]{imagenes/logo_universidad_biobio.png}
    };
    \node[anchor=west, text=white, font=\scriptsize\bfseries] at ([xshift=0.74cm,yshift=-0.24cm]current page.north west) {Ciencia de Datos para Economía y Gestión};
    \node[anchor=east, text=arena, font=\scriptsize] at ([xshift=-3.85cm,yshift=-0.24cm]current page.north east) {Tarea 1 | Limpieza de datos mineros};
    \node[anchor=east] at ([xshift=-0.34cm,yshift=-0.24cm]current page.north east) {\progressdots};
...
\begin{frame}[plain]
  ...
  \begin{tikzpicture}[remember picture,overlay]
    ...
    \node[anchor=north east, fill=white, fill opacity=0.90, text opacity=1, rounded corners=3pt, inner sep=4pt] at ([xshift=-0.55cm,yshift=-0.45cm]current page.north east) {
      \includegraphics[height=1.65cm]{imagenes/logo_universidad_biobio.png}
    };
  \end{tikzpicture}
...
\begin{frame}[fragile]
  \bgphoto{imagenes/catodo_cobre.jpg}{white}{0.86}
  \copperband
  \vspace{0.34cm}
  \slidetitle{mineral}{5. ETL: extracción}

  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.58\textwidth}
\begin{lstlisting}[style=codexpython]
...
\end{lstlisting}
    ...
    \begin{column}{0.36\textwidth}
      \darkpanel{0.95\textwidth}{
        {\color{arena}\bfseries Qué hace esta fase}\\[4pt]
        {\color{white}\small Se toma el Excel original de COCHILCO y se carga en Python. No se corrige nada todavía: el objetivo es conservar una copia bruta para revisar cómo venía el archivo.}
      }
      \vspace{0.18cm}
      \panel{0.95\textwidth}{
        \small El parámetro \texttt{header=6} indica que los nombres de columnas reales comienzan en esa fila del Excel.
      }
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \copperband
  \vspace{0.34cm}
  \slidetitle{mineral}{6. ETL: transformación}
...
\end{lstlisting}
    ...
    \begin{column}{0.36\textwidth}
      \darkpanel{0.95\textwidth}{
        {\color{arena}\bfseries Limpieza estructural}\\[4pt]
        {\color{white}\small Se trabaja sobre una copia, se eliminan filas totalmente vacías y se estandarizan los nombres de columnas para que sean fáciles de usar en código.}
      }
      \vspace{0.18cm}
      \panel{0.95\textwidth}{
        \small Ejemplo: una columna con espacios, mayúsculas o acentos queda convertida a un nombre simple como \texttt{total\_chile}.
      }
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \copperband
  \vspace{0.34cm}
  \slidetitle{mineral}{7. ETL: transformación analítica}
...
\end{lstlisting}
    ...
    \begin{column}{0.36\textwidth}
      \darkpanel{0.95\textwidth}{
        {\color{arena}\bfseries Preparación para analizar}\\[4pt]
        {\color{white}\small Se dejan solo meses con producción nacional informada y se crea el formato largo: una fila por mes y por faena.}
      }
      \vspace{0.18cm}
      \panel{0.95\textwidth}{
        \small Esta transformación permite comparar faenas, calcular rankings y hacer histogramas por empresa/faena.
      }
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[fragile]
  \bgphoto{imagenes/camion_mina.jpg}{carbon}{0.68}
  \copperband
  \vspace{0.34cm}
  \slidetitle{white}{8. ETL: carga y resultados}
...
\end{lstlisting}
    ...
    \begin{column}{0.36\textwidth}
      \darkpanel{0.95\textwidth}{
        {\color{arena}\bfseries Qué se entrega}\\[4pt]
        {\color{white}\small La fase de carga guarda los productos finales: dataset limpio en formato ancho, dataset largo y resumen por faena.}
      }
      \vspace{0.18cm}
      \panel{0.95\textwidth}{
        \small Estos archivos son la base para los gráficos, tablas, notebook ejecutado, informe y presentación.
      }
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}
  ...
  \copperband
  \vspace{0.4cm}
  \slidetitle{white}{9. Radiografía del dataset}
...
\begin{frame}
  ...
  \copperband
  \vspace{0.32cm}
  \slidetitle{mineral}{10. Distribución de la producción nacional}
...
\begin{frame}
  ...
  \copperband
  \vspace{0.32cm}
  \slidetitle{mineral}{11. Concentración por empresa y faena}
...
\begin{frame}
  ...
  \copperband
  \vspace{0.32cm}
  \slidetitle{white}{12. Relaciones con la producción nacional}
...
\begin{frame}
  ...
  \copperband
  \vspace{0.34cm}
  \slidetitle{mineral}{13. Correlaciones relevantes}
...
\begin{frame}
  ...
  \copperband
  \vspace{0.34cm}
  \slidetitle{mineral}{14. Valores faltantes y meses sin dato}
...
\begin{frame}
  ...
  \copperband
  \vspace{0.32cm}
  \slidetitle{mineral}{15. Outliers: no todo valor extremo es error}
...
\begin{frame}
  ...
  \copperband
  \vspace{0.34cm}
  \slidetitle{white}{16. Asimetría y concentración}
...
\begin{frame}
  ...
  \copperband
  \vspace{0.42cm}
  \slidetitle{white}{17. Conclusiones}
...
\begin{frame}
  ...
  \copperband
  \vspace{0.35cm}
  \slidetitle{mineral}{18. Fuentes y créditos visuales}
